...
If `data-tooltip-content` is unset (or has an empty value) on the anchor element, and the `content`, `render`, and `children` props on the tooltip are also unset (or have empty values), the tooltip is not shown by default.
...
This is not specific to Next.js. `react-tooltip` is a client-side component, so the same rule applies to any React Server Components setup: render the tooltip from a client boundary.
